...
const roundDecimals = (arr) => {
    // write your code here
     let result = [];
     arr.forEach((item) => {
        const rounded = item >= 0 ? Math.floor(item + 0.5) : Math.ceil(item - 0.5);
        result.push(rounded);
     });
        return result;
...
const employeesBonus = (arr) => {
    // write your code here
    arr.forEach((iteam) => {
        if (iteam.workHours > 8) {
            iteam.salary = parseInt(iteam.salary);
            iteam.salary += 100;
            iteam.salary = `${iteam.salary}$`;
        } else {
            iteam.salary = parseInt(iteam.salary);
            iteam.salary += 50;
            iteam.salary = `${iteam.salary}$`;
